...
class RoboEyes:
    ...
    def _safe_constraint_x(self, margin=0):
        # Use NEXT values (targets), not current, to avoid easing overshoot
        lw = max(0, self.eyeLwidth_next)
        rw = max(0, self.eyeRwidth_next)
        sp = max(0, self.space_between_next)

        max_x = self.screen_width - (lw + sp + rw) - margin
        return max(0, max_x)

    def _safe_constraint_y(self, margin=0):
        lh = max(0, self.eyeLheight_next)  # next target height
        max_y = self.screen_height - lh - margin
        return max(0, max_y)
